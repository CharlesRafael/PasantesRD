...
// import { Bell, UserPlus, Eye, CheckCircle, XCircle, Clock, User } from 'lucide-react';
...
import Link from 'next/link';
import Image from 'next/image';
import { Bell, UserPlus, Eye, CheckCircle, XCircle, Clock, User, Briefcase, Notebook, Menu, X } from 'lucide-react';

// Student Notification Bell Component (keep exactly as is)
const StudentNotificationBell = () => {
  const [notifications, setNotifications] = useState([]);
  const [unreadCount, setUnreadCount] = useState(0);
  const [showDropdown, setShowDropdown] = useState(false);
    ...
    <div className="min-h-screen p-0 flex flex-col bg-[#1C274D] text-white">
      <header className="">
        <nav className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          {/* Mobile Navigation Icons - shown only on mobile */}
          <div className="flex md:hidden items-center gap-4">
            {/* Briefcase Icon - Internships */}
            <Link href="/internship_search" className="p-2 hover:text-blue-600 transition-colors">
              <Briefcase className="w-5 h-5" />
            </Link>

            {/* Notebook Icon - Companies */}
            <Link href="/company_directory" className="p-2 hover:text-blue-600 transition-colors">
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-[#1C274D] border-t border-gray-700 px-6 py-4">
